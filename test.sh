...
WYNIKI="wyniki_$(date +%Y%m%d_%H%M).csv"
echo "uzytkownicy,trans_sek,sr_czas_s,najdluzszy_s,dostepnosc_pct" > $WYNIKI

for USERS in 10 50 100 250 500; do
    echo ">>> Test: $USERS uzytkownikow..."

    sudo -u postgres psql -d system_biletowy -c \
    ...
    sudo -u postgres psql -d system_biletowy -c "
        SELECT round(mean_exec_time::numeric,2) AS sr_ms, calls,
               left(query,60) AS kwerenda
